...
_DISCLAIMER = (
    "답변 전체에서 💡·📌·> 인용·※ 참고·추가 설명·불확실성 주석 등 어떤 추가 블록도 절대 넣지 마. "
    "`<small>` 태그가 답변의 진짜 마지막 줄이어야 해 — 그 뒤에는 아무것도 오면 안 돼.\n"
    "아래 형식으로 출력해 (실제 값으로 채우고, URL은 반드시 마크다운 링크 [텍스트](URL) 형식으로):\n"
    '<small style="color:gray">출처: 실제_출처_명시. '
    "정확한 내용은 [카드사명 공식 홈페이지](실제_URL)나 고객센터(☎ 실제_대표번호)를 통해 확인하세요.</small>"
)
