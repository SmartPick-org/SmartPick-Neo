...
QUERIES_STANDALONE: dict[str, str] = {
    "reviews": (
        "이 카드의 실사용자 후기를 검색해서 아래 형식으로 답해줘. "
        "카드 혜택·장점 설명은 하지 마.\n\n"
        "## 전반적 평가\n"
        ...
        '<small style="color:gray">검색 기반 실사용자 후기를 요약한 정보입니다. 개인 경험에 따라 다를 수 있으며, 최신 정보는 카드사 공식 채널을 통해 확인하세요.</small>'
    ),
    "how_to_apply": (
        ...
        '<small style="color:gray">발급 조건 및 서류는 카드사 정책에 따라 변경될 수 있습니다. 정확한 내용은 카드사 공식 홈페이지나 고객센터(☎ 카드 뒷면 번호)를 통해 확인하세요.</small>'
    ),
}
...
QUERIES_DETAILS: dict[str, str] = {
    "credit_fees": (
        "이 카드의 연회비와 이용형태별 수수료율을 아래 형식으로 답해줘.\n\n"
        "## 요약\n"
        "연회비와 주요 수수료를 2~3문장으로 간략히 요약해줘. 카드를 처음 쓰는 사람도 이해하기 쉽게 설명해줘.\n\n"
        "## 상세 내용\n"
        "아래 항목을 표(table) 형식으로 정리해줘. 약관에 없는 항목은 표에서 빼줘:\n\n"
        "| 항목 | 수수료율 |\n"
        "|------|----------|\n"
        "| 일시불 | |\n"
        "| 할부 수수료율 (연, 최저~최고) | |\n"
        ...
        "| 연체이자율 | |\n\n"
        '<small style="color:gray">카드사 공식 약관 및 상품설명서 기준입니다. 실제 적용 수수료율은 개인 신용도에 따라 다를 수 있으며, 최신 정보는 카드사 공식 홈페이지나 고객센터를 통해 확인하세요.</small>'
    ),
    "international_fees": (
        "이 카드의 해외 사용 수수료를 아래 형식으로 답해줘.\n\n"
        "## 요약\n"
        "해외에서 이 카드를 쓸 때 붙는 수수료를 2~3문장으로 간략히 요약해줘. 해외여행 가는 일반 사용자가 이해하기 쉽게 설명해줘.\n\n"
        "## 상세 내용\n"
        "아래 항목을 표(table) 형식으로 정리해줘. 약관에 없는 항목은 표에서 빼줘:\n\n"
        "| 항목 | 내용 |\n"
        "|------|------|\n"
        ...
        "| 원화 청구 시 환율 기준 | |\n\n"
        '<small style="color:gray">카드사 공식 약관 및 상품설명서 기준입니다. 실제 적용 수수료 및 환율은 결제 시점에 따라 다를 수 있으며, 최신 정보는 카드사 공식 홈페이지나 고객센터를 통해 확인하세요.</small>'
    ),
    "late_payment": (
        "이 카드 이용대금을 연체하면 어떻게 되는지 아래 형식으로 답해줘.\n\n"
        "## 요약\n"
        "연체 시 가장 중요하게 알아야 할 사항을 2~3문장으로 요약해줘. 카드를 처음 쓰는 사람도 이해하기 쉽게 설명해줘.\n\n"
        "## 상세 내용\n"
        "아래 항목을 불릿 포인트(•)로 정리해줘. 약관에 없는 항목은 빼줘:\n"
        "• 연체이자율 계산 방식 (정상이자율 + 가산금리, 최고 연이율)\n"
        "• 일시불·무이자할부 연체 시 적용 기준\n"
        "• 연체로 인한 불이익 (신용점수 하락, 카드 이용 정지, 한도 감액, 계약 해지 등)\n"
        "• 기한의 이익 상실 주요 사유\n\n"
        '<small style="color:gray">카드사 공식 약관 기준입니다. 연체는 신용점수에 영향을 줄 수 있습니다. 정확한 내용은 카드사 공식 홈페이지나 고객센터를 통해 확인하세요.</small>'
    ),
    "revolving": (
        "이 카드의 리볼빙(일부결제금액이월약정) 서비스를 아래 형식으로 설명해줘.\n\n"
        "## 요약\n"
        "리볼빙이 무엇인지, 이용 시 주의할 점을 2~3문장으로 쉽게 설명해줘. 리볼빙을 처음 접하는 사람도 이해할 수 있게 해줘.\n\n"
        "## 상세 내용\n"
        "아래 항목을 불릿 포인트(•)로 정리해줘. 약관에 없는 항목은 빼줘:\n"
        "• 리볼빙 수수료율 범위 (연 최저~최고)\n"
        "• 약정결제비율과 최소결제비율의 차이\n"
        ...
        '<small style="color:gray">카드사 공식 약관 기준입니다. 리볼빙은 높은 수수료가 발생할 수 있어 신중한 이용이 필요합니다. 정확한 내용은 카드사 공식 홈페이지나 고객센터를 통해 확인하세요.</small>'
    ),
}
