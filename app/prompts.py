...
6. 1순위 카드에 대해서만 설명한다. 2, 3순위는 언급하지 않는다.
7. Compact Digest의 혜택 정보를 참고하되, 원문을 그대로 복사하지 말 것. 자연스러운 문장으로 재구성할 것.
8. [예상 혜택 계산 결과]에 "※ 고비중 혜택 참고"가 있으면, 해당 혜택이 전체 할인에서
   큰 비중을 차지함을 자연스럽게 언급하고, 해당 혜택의 사용 전제 조건(특정 매장, 시간대,
   플랫폼 등)을 안내하여 유저가 현실적인 혜택 수준을 가늠할 수 있게 한다.
   없으면 이 규칙은 무시한다.
...
1. 혜택이 적용되는 카테고리 기반 자연스러운 도입
2. 1순위 카드명과 해당 카테고리 연결
3. 카테고리별 혜택 간단 언급 (고비중 혜택이 있다면 전제 조건 주의 안내 포함)
4. 총 예상 혜택 (월/연)
...
교통과 식비 등 자주 소비하시는 곳에서
혜택을 많이 받을 수 있는 카드를 중심으로 추천드렸어요.

특히 'Mr.Life 카드'는 식비에서 높은 할인 혜택을 제공하는데,
이 중 인테이크몰 20% 할인이 전체 할인의 상당 부분을 차지해요.
만약 인테이크몰을 자주 이용하지 않으신다면 실제 체감 혜택은 다를 수 있으니 참고해 주세요.

현재 소비 기준으로 월 약 4만원,
연간 약 48만원 수준의 혜택이 예상돼요.
"""
...
EXPLAIN_PROMPT = EXPLAIN_PROMPT_V3  # V4 병행 테스트 후 전환
