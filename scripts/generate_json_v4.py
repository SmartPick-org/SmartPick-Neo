...
LLM(solar-pro2)으로 v4 스키마 JSON을 생성합니다.

사용법: python -m scripts.generate_json_v4
옵션:
...
CONVERT_V4_PROMPT = """너는 신용카드 약관/설명서 마크다운을 읽고, 카드 혜택 계산 엔진에서 사용할 구조화된 JSON을 생성하는 금융 데이터 파서이다.
...
subcategory는 아래 목록 중 하나로만 선택:

Traffic
- Transit: 대중교통(버스/지하철/철도/KTX/SRT)
- Taxi: 택시
- Fuel: 주유(휘발유/경유/주유소)
- EV_Charge: 전기차 충전
- Parking: 주차/주차장
...
- OTT_Subscription: OTT/구독
- LeisureSports: 레저/스포츠
...
- ManagementFee: 관리비
- Household: 생활서비스(세탁/청소)
- TaxInsurancePension: 세금/보험/연금
...
def convert_card(card_key: str, group: dict) -> dict | None:
    """카드 그룹의 마크다운들을 합쳐 LLM으로 v4 JSON을 생성합니다."""
    # 마크다운 합치기
    md_parts = []
    ...
    prompt = CONVERT_V4_PROMPT.format(
