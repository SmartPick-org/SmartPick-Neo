...
def _details_prompt(topic: str, summary: str, details: str) -> str:
    ...
    모든 세부 쿼리는 ## 요약 → ## 상세 내용 → <small> 구조를 공유한다."""
    return (
        f"{topic} 아래 형식으로만 답해줘. "
        "섹션 제목은 반드시 `##` 헤딩으로 출력하고 불릿 포인트로 쓰지 마. "
        "그 외 어떤 추가 블록·인용·※ 참고도 넣지 마.\n\n"
        f"## 요약\n{summary}\n\n"
        f"## 상세 내용\n약관·상품설명서에 없는 항목은 빼줘:\n\n{details}\n\n"
        + _DISCLAIMER
    )
...
_DISCLAIMER = (
    "답변 맨 마지막 줄은 반드시 아래 형식의 `<small>` 태그 한 줄로만 끝내줘. "
    "표나 불릿 목록이 끝난 뒤 💡·📌·> 인용·※ 참고 등 어떤 추가 블록도 절대 넣지 마 — `<small>` 태그가 진짜 마지막 줄이어야 해:\n"
    '<small style="color:gray">[참고한 출처(약관·상품설명서·검색 결과 등 실제 사용한 출처를 명시)]. '
    "정확한 내용은 [이 카드사의 실제 공식 홈페이지 URL]이나 고객센터(☎ [이 카드사의 실제 대표 번호])를 통해 확인하세요.</small>"
)
...
QUERIES_STANDALONE: dict[str, str] = {
    "reviews": (
        ...
        "답변은 반드시 아래 세 섹션과 `<small>` 태그로만 구성해야 해 — 그 외 어떤 섹션·블록·인용도 추가하지 마.\n\n"
        "## 전반적 평가\n"
    ...
    "how_to_apply": (
        ...
        "답변은 반드시 아래 세 `##` 헤딩 섹션과 `<small>` 태그로만 구성해 — 각 섹션 제목은 불릿 포인트가 아닌 `##` 헤딩으로 출력하고, 그 외 어떤 추가 블록·인용·※ 참고도 넣지 마.\n\n"
        "## 신청 URL\n"
