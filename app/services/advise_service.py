...
_DISCLAIMER = (
    "답변 전체에서 💡·📌·※ 참고·추가 설명·불확실성 주석·HTML 태그(꺾쇠괄호로 감싼 태그)·인라인 스타일·"
    "~~취소선~~ 문법 등 "
    "어떤 추가 블록도 절대 넣지 마. "
...
QUERIES_DETAILS: dict[str, str] = {
    "credit_fees": _details_prompt(
        ...
        summary=(
            "아래 내용을 2~3문장의 문장형 prose로 자연스럽게 풀어서 설명해줘 "
            "(번호·불릿 금지, 숫자 나열 대신 사용자 입장에서 와닿게):\n"
            "연회비 구간이 여러 개라면 비싼 타입을 고르면 어떤 혜택이 추가되는지, "
            "그리고 연체이자율을 기준으로 '50만 원을 한 달 연체하면 이자가 얼마 붙는지' "
            "실제 금액으로 계산해줘."
        ),
        details=(
    ...
    "international_fees": _details_prompt(
        ...
        summary=(
            "아래 내용을 2~3문장의 문장형 prose로 자연스럽게 풀어서 설명해줘 "
            "(번호·불릿 금지, 처음 해외에서 카드를 쓰는 사람이 이해하기 쉽게):\n"
            "국제브랜드 수수료와 해외서비스 수수료가 각각 누가 떼어가는 비용인지 짚어주고, "
            "100달러 결제 시 환율 외에 실제 수수료가 원화로 얼마나 더 붙는지 계산해줘. "
            "이 수수료는 해외 현지뿐 아니라 국내 외화 결제(해외직구 등)에도 똑같이 적용된다는 점도 덧붙여줘."
        ),
        details=(
            "아래 항목을 표(table) 형식으로 정리해줘:\n\n"
            "| 항목 | 내용 |\n"
    ...
    "late_payment": _details_prompt(
        ...
        summary=(
            "아래 내용을 2~3문장의 문장형 prose로 자연스럽게 풀어서 설명해줘 "
            "(번호·불릿 금지, 처음 카드를 쓰는 사람이 와닿게):\n"
            "연체이자율을 기준으로 '50만 원을 한 달 연체하면 연체이자가 얼마 붙는지' "
            "실제 금액으로 계산해주고, 연체가 길어질수록 이자 → 신용점수 하락 → 카드 정지 "
            "순으로 벌어지는 흐름을 이어서 짚어줘."
        ),
        details=(
    ...
    "revolving": _details_prompt(
        ...
        summary=(
            "아래 내용을 2~3문장의 문장형 prose로 자연스럽게 풀어서 설명해줘 "
            "(번호·불릿 금지, 리볼빙을 처음 접하는 사람에게 체감되게):\n"
            "리볼빙이 무엇인지, 왜 쓰는지(자금 부족 해결)와 왜 위험한지(수수료 누적)를 함께 짚어주고, "
            "이 카드 수수료율로 '100만 원을 한 달 이월하면 수수료가 얼마인지' 실제 금액을 계산해줘. "
            "같은 잔액을 3개월 연속 이월하면 총 수수료가 얼마로 불어나는지도 이어서 덧붙여줘."
        ),
        details=(
            "아래 항목을 불릿 포인트(•)로 정리해줘:\n"
            "• 리볼빙 수수료율 범위 (연 최저~최고)\n"
