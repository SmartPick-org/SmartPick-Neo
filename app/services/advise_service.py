...
def _details_prompt(topic: str, summary: str, details: str) -> str:
    ...
    return (
        ...
        "섹션 제목은 반드시 `##` 헤딩으로 출력해 (섹션 제목을 불릿으로 쓰지 마). "
...
QUERIES_DETAILS: dict[str, str] = {
    "credit_fees": _details_prompt(
        ...
        summary=(
            "아래 내용을 불릿 포인트(•) 4~6개로 누구나 이해할 수 있는 쉬운 표현으로 정리해줘:\n"
            "• 연회비 종류와 각 타입에서 받을 수 있는 혜택 차이\n"
            "• 일시불 수수료율\n"
            "• 할부 수수료율 범위 (최저~최고)\n"
            "• 연체이자율 기준, '50만 원 한 달 연체 시 이자 실제 금액' 계산 포함\n"
            "• 단기카드대출(현금서비스) 수수료율 범위"
        ),
        details=(
    ...
    "international_fees": _details_prompt(
        ...
        summary=(
            "아래 내용을 불릿 포인트(•) 4~6개로 누구나 이해할 수 있는 쉬운 표현으로 정리해줘:\n"
            "• 국제브랜드 수수료가 무엇인지, 누가 가져가는 비용인지\n"
            "• 해외서비스 수수료가 무엇인지, 누가 가져가는 비용인지\n"
            "• 100달러 결제 시 환율 외에 실제로 추가되는 수수료 금액 계산\n"
            "• 해외직구 등 국내 외화 결제에도 동일하게 적용된다는 점\n"
            "• 현금서비스 해외 이용 수수료 및 면제 조건"
        ),
        details=(
            "아래 항목을 표(table) 형식으로 정리해줘:\n\n"
            "| 항목 | 내용 |\n"
    ...
    "late_payment": _details_prompt(
        ...
        summary=(
            "아래 내용을 불릿 포인트(•) 4~6개로 누구나 이해할 수 있는 쉬운 표현으로 정리해줘:\n"
            "• 연체이자율이 몇 %인지\n"
            "• '50만 원을 한 달 연체하면 이자가 얼마인지' 실제 금액으로 계산\n"
            "• 연체가 계속되면 어떤 일이 순서대로 벌어지는지 (이자 → 신용점수 하락 → 카드 정지)\n"
            "• 연체로 인해 생기는 주요 불이익 목록"
        ),
        details=(
    ...
    "revolving": _details_prompt(
        ...
        summary=(
            "아래 내용을 불릿 포인트(•) 4~6개로 누구나 이해할 수 있는 쉬운 표현으로 정리해줘:\n"
            "• 리볼빙이 무엇인지 (결제 금액 일부를 다음 달로 미루는 서비스)\n"
            "• 왜 쓰는지 (일시적 자금 부족 해결)와 왜 위험한지 (수수료 누적)\n"
            "• '100만 원을 한 달 이월하면 수수료가 얼마인지' 실제 금액으로 계산\n"
            "• 3개월 연속 이월하면 총 수수료가 얼마로 불어나는지 계산\n"
            "• 이월잔액이 신용점수에 미치는 영향"
        ),
        details=(
            "아래 항목을 불릿 포인트(•)로 정리해줘:\n"
