"""
마크다운 기반 카드 JSON v5 생성 스크립트

markdown_upstage/ 하위의 마크다운 파일들을 카드별로 그룹핑한 뒤,
LLM(solar-pro2)으로 v5 스키마 JSON을 생성합니다.

사용법: python -m scripts.generate_json_v5
옵션:
...
CONVERT_V5_PROMPT = """너는 신용카드 약관/설명서 마크다운을 읽고, 카드 혜택 계산 엔진에서 사용할 구조화된 JSON을 생성하는 금융 데이터 파서이다.
...
{{
  ...
  "benefits": [
    {{
      ...
      "subcategory": "표준 세부 카테고리 (아래 목록 참조)",
...
subcategory는 아래 목록 중 하나로만 선택 (snake_case):

Traffic
- transit: 시내버스, 지하철, 철도, KTX, SRT
- express_bus: 고속버스, 시외버스
- taxi: 택시, 택시 호출앱
- fuel: 주유소, LPG, 리터당 할인
- ev_charge: 전기차 충전 (Reserved)
- parking: 주차장, 주차 할인
- toll: 통행료, 하이패스 (Reserved)
- maintenance: 차량 정비, 세차, 엔진오일

Shopping
- online: 온라인쇼핑, 이커머스, 패션앱
- dept_store: 백화점, 아울렛
- mart: 대형마트, 슈퍼
- convenience: 편의점 (CU, GS25 등)
- home_shopping: 홈쇼핑 (Reserved)
- duty_free: 면세점
- daily_goods: 생활용품, 잡화 (Reserved)
- beauty: 뷰티, 미용, 올리브영, 미용실

Food
- restaurant: 외식, 음식점, 식음료
- delivery: 배달앱 (배민, 요기요 등)
- fast_food: 패스트푸드, 버거
...
Cultural
- cinema: 영화관 (CGV, 메가박스, 롯데시네마)
- performance: 공연, 전시 (Reserved)
- books: 도서, 서점, 온라인서점
- music: 음악 스트리밍 (멜론, 지니 등)
- ott: OTT, 디지털 구독 (넷플릭스, 유튜브, 티빙 등)
- theme_park: 놀이공원, 테마파크 (에버랜드, 롯데월드 등)
- leisure_sports: 레저, 스포츠 (골프, 테니스 등)

Travel
- airline: 항공, 마일리지 적립
- hotel: 호텔, 숙박 예약 플랫폼
- travel_agency: 여행사, 여행 플랫폼 (Trip.com, KKday 등)
- overseas: 해외 결제, 해외직구 캐시백
- lounge_valet: 공항 라운지, 발레파킹
- rental: 렌터카, 카셰어링
- roaming: 로밍, 해외 데이터, eSIM

Life
- telecom: 이동통신, 인터넷 요금
- utility: 전기, 가스, 수도, 공과금
- housing: 관리비, 아파트 관리비 (Reserved)
- daily_service: 세탁, 청소, 정수기렌탈 등
- insurance_tax: 보험료, 국세, 지방세, 연금
- office: 문구, 사무용기기, 보안/용역 (Biz 카드 대상)

EduHealth
- hospital: 병원, 한의원, 치과, 안과
- pharmacy: 약국
- education: 학원, 독서실, 교육
- fitness: 피트니스, 헬스, 요가
- medical_detail: 의료 세부 (Reserved)
...
def validate_v5(data: dict) -> dict:
    """v5 JSON 후처리 검증."""
    # card_meta 필수 필드 확인
...
def convert_card(card_key: str, group: dict) -> dict | None:
    """카드 그룹의 마크다운들을 합쳐 LLM으로 v5 JSON을 생성합니다."""
    # 마크다운 합치기
    md_parts = []
    ...
    prompt = CONVERT_V5_PROMPT.format(
...
def main():
    parser = argparse.ArgumentParser(description="마크다운 → JSON v5 변환")
