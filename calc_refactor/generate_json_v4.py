...
4. 복합 카테고리 혜택 및 한도 공유(SHARED_LIMIT) 정밀 판정 지침:
   - 다음 패턴이 발견되면 반드시 `benefit_groups`에 `SHARED_LIMIT` 객체를 생성하고 동일한 `group_id`로 묶어라.
   - [패턴 A: 행 내부 통합] 한 행에 여러 카테고리가 나열된 경우 (예: '대중교통, 택시, 통신 | 10% | 5천원 통합'): 이 셋을 개별 혜택으로 분리하되 반드시 하나의 공유 그룹으로 묶는다. (예: KB 청춘대로 톡톡)
   - [패턴 B: 지배적 문맥/문장] 특정 섹션 상단이나 하단에 "대상 영역 통합", "영역 통합", "합산하여 적용" 등의 문구가 있는 경우: 해당 범위 내 모든 혜택은 하나의 `group_id`를 공유한다. (예: 현대 M '대상 영역 통합 월 1만 M포인트', 신한 처음 '영역 통합 월 최대 1,000 포인트')
   - [패턴 C: 상단 마스터 한도 표] 문서 초반에 서비스별 통합 한도 요약 표가 먼저 제시되는 경우: 하단의 상세 내역들은 이 상단 표의 그룹 ID에 연결되어야 한다. (예: 신한 처음)
   - [패턴 D: 명시적 태그] 마크다운에 `[한도공유_이름]` 또는 `[한도공유:이름]` 태그가 있다면 최우선적으로 동일한 `group_id`를 부여한다.
   - 단, "(위와 동일)", "건당 한도" 수준의 단순 텍스트만 있고 통합/합산 키워드가 없으면 개별 한도로 간주한다.
...
6. 패키지형 선택 혜택 및 공유 한도의 직교 태그(Orthogonal Tags) 파싱 지침 (최우선순위 적용):
   - 마크다운 원문에 `[그룹이름:선택지명]` (예: `[선택그룹1:선택지A]`) 처럼 배타적 패키지 선택을 지시하는 태그가 존재할 경우:
     * `benefit_groups`에 `group_type: "SELECTIVE_GROUP"` 객체를 만들고 하위 `choices` 배열에 패키지 정보(choice_id, choice_name)를 구성하라.
     * 해당 `benefits` 항목 내부에 `selective_choice: {{ "group_id": "생성된 그룹 ID", "choice_id": "선택지의 ID" }}` 를 반드시 부여하라.
   - 원문에 `[한도공유_이름]` 처럼 공유 한도를 특정하는 직교 태그가 있다면 관련된 혜택들에 똑같은 `group_id`를 부여하고 `benefit_groups`에 `SHARED_LIMIT` 객체를 별도로 등록하라.

[추론 프로세스 가이드]
- 마크다운을 읽을 때 혜택의 나열 순서뿐만 아니라, 앞뒤에 붙은 '통합/합산' 문구가 어느 혜택들까지 영향을 미치는지(Scope) 먼저 판단하라.
- 특히 현대카드의 '대상 영역' 텍스트나 신한카드의 '방과 후 2% 추가 적립: 영역 통합' 같은 문구는 개별 혜택들의 상위에 존재하는 지배적 제약 조건(Governing Constraint)임을 인지하라.
